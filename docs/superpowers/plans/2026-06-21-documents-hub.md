# Documents Hub Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development. Steps use checkbox (`- [ ]`) syntax.

**Goal:** A **Documents** screen that organizes generated artifacts by kind (Résumés · Cover letters · Briefs · Outreach) plus a Communications view — reusing the existing `ArtifactCard`.

## Global Constraints
- Reuse existing fetchers/components: `fetchArtifacts`, `fetchCommunications`, `<ArtifactCard artifact onChanged>`. No backend changes.
- Job Hunter tokens; literal Tailwind classes. Frontend build must pass. Keep all canvas destinations reachable; add `documents` to the union + rail + a render branch.
- Do NOT invoke any finishing/branch skill — stop after committing.

---

### Task 1: DocumentsTab + wiring

**Files:** Create `frontend/app/components/DocumentsTab.tsx`; Modify `frontend/app/page.tsx`, `frontend/app/components/IconRail.tsx`. (Verify `fetchCommunications` exists in api.ts; if not, add a tiny `fetchCommunications(): Promise<Communication[]>` GET `/api/communications`.)

- [ ] **Step 1: Create `DocumentsTab.tsx`**

```tsx
"use client";

import { useCallback, useEffect, useState } from "react";
import { Artifact, Communication, fetchArtifacts, fetchCommunications } from "@/lib/api";
import ArtifactCard from "./ArtifactCard";
import FetchError from "./FetchError";

const GROUPS: { key: string; label: string; kinds: string[] }[] = [
  { key: "all", label: "All", kinds: [] },
  { key: "resumes", label: "Résumés", kinds: ["cv"] },
  { key: "covers", label: "Cover letters", kinds: ["cover_letter"] },
  { key: "briefs", label: "Briefs", kinds: ["research_brief", "fit_analysis"] },
  { key: "outreach", label: "Outreach", kinds: ["outreach", "pitch", "proposal"] },
  { key: "comms", label: "Communications", kinds: [] },
];

export default function DocumentsTab() {
  const [artifacts, setArtifacts] = useState<Artifact[]>([]);
  const [comms, setComms] = useState<Communication[]>([]);
  const [error, setError] = useState(false);
  const [group, setGroup] = useState("all");

  const load = useCallback(() => {
    fetchArtifacts()
      .then((a) => { setArtifacts(a); setError(false); })
      .catch(() => { setArtifacts([]); setError(true); });
    fetchCommunications().then(setComms).catch(() => setComms([]));
  }, []);
  useEffect(() => { load(); }, [load]);

  if (error) return <FetchError onRetry={load} />;

  const g = GROUPS.find((x) => x.key === group)!;
  const docs = g.kinds.length ? artifacts.filter((a) => g.kinds.includes(a.kind)) : artifacts;

  return (
    <div className="min-h-0 flex-1 overflow-y-auto p-5">
      <h1 className="text-[22px] font-bold tracking-tight text-ink">Documents</h1>
      <p className="mb-4 text-[13.5px] text-ink-muted">{artifacts.length} generated documents · {comms.length} communications</p>

      <div className="mb-4 flex flex-wrap gap-1.5">
        {GROUPS.map((x) => (
          <button
            key={x.key}
            onClick={() => setGroup(x.key)}
            className={`rounded-md px-3 py-1.5 text-[12.5px] font-semibold transition ${
              group === x.key ? "bg-accent text-white" : "border border-line bg-surface text-ink-muted hover:bg-accent-tint hover:text-accent"
            }`}
          >
            {x.label}
          </button>
        ))}
      </div>

      {group === "comms" ? (
        comms.length === 0 ? (
          <p className="text-[13px] text-ink-subtle">No communications logged.</p>
        ) : (
          <div className="flex flex-col gap-2.5">
            {comms.map((c) => (
              <div key={c.id} className="rounded-xl border border-line bg-surface p-4">
                <div className="flex items-center gap-2">
                  <span className="rounded-xs bg-accent-tint px-1.5 py-0.5 text-[10.5px] font-semibold uppercase text-accent">{c.channel}</span>
                  <span className="rounded-xs bg-surface-sunk px-1.5 py-0.5 text-[10.5px] text-ink-muted">{c.direction}</span>
                  <span className="ml-auto font-mono text-[10px] text-ink-subtle">{new Date(c.occurred_at).toLocaleDateString()}</span>
                </div>
                {c.subject && <div className="mt-1.5 text-[13.5px] font-semibold text-ink">{c.subject}</div>}
                {c.body && <p className="mt-1 line-clamp-3 text-[12.5px] leading-snug text-ink-body">{c.body}</p>}
              </div>
            ))}
          </div>
        )
      ) : docs.length === 0 ? (
        <p className="text-[13px] text-ink-subtle">No documents in this group yet — generate them from an opportunity (CV tailor, cover letter, …).</p>
      ) : (
        <div className="flex flex-col gap-3">
          {docs.map((a) => (
            <ArtifactCard key={a.id} artifact={a} onChanged={load} />
          ))}
        </div>
      )}
    </div>
  );
}
```

- [ ] **Step 2: Wire into shell**

- `page.tsx`: add `| "documents"` to the `canvasTab` union; `import DocumentsTab from "./components/DocumentsTab";`; render branch `) : canvasTab === "documents" ? ( <DocumentsTab /> )`.
- `IconRail.tsx`: there is currently a "Documents"-labeled item aliased to `workspace`; REPLACE that alias so a real `documents` entry exists. Specifically, in the ITEMS array change the item `{ key: "interviews-cal", label: "Documents", icon: I("M5 3h6l5 5v10H5z M11 3v5h5") }` to `{ key: "documents", label: "Documents", icon: I("M5 3h6l5 5v10H5z M11 3v5h5") }`, and REMOVE the de-dupe alias line in the component body (`const items = ITEMS.map(... interviews-cal → workspace ...)`) — replace it with `const items = ITEMS.filter((it, i, a) => a.findIndex((x) => x.key === it.key) === i);` so `documents` is its own destination and `workspace` (the last item) stays.

- [ ] **Step 3: Build** — `npm --prefix frontend install` then `npm --prefix frontend run build` → succeeds. Verify both `documents` and `workspace` are reachable from the rail.

- [ ] **Step 4: Commit**

```bash
git add frontend/app/components/DocumentsTab.tsx frontend/app/page.tsx frontend/app/components/IconRail.tsx
git commit -m "feat(ui): Documents hub — artifacts by kind (résumés/cover/briefs/outreach) + communications"
```
